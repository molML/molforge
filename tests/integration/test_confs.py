...
@pytest.fixture
def test_data():
    """Sample SMILES data: three valid molecules and one invalid.

    Kept at four molecules because the OpenEye backend test asserts a fixed
    molecules_processed count; the RDKit speedup comes from the small
    ``FAST_MAX_CONFS`` rather than from trimming this fixture.
    """
    return pd.DataFrame({
        'smiles': [
            'CCO',  # Ethanol - simple
...
def conformer_test_data():
    """One stereochemically-defined drug-like molecule plus one invalid SMILES.

    A single valid molecule is enough to verify pickle/extraction integrity;
    stereochemistry is specified so the OpenEye OMEGA (classic) path can also
    embed it. The invalid row exercises graceful failure and exclusion.
    """
    return pd.DataFrame({
        'smiles': [
        ...
        'molecule_id': ['s_ibuprofen', 'invalid']
    })
