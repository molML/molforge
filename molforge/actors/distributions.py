...
class CurateDistribution(BaseActor):
    ...
    def output_columns(self) -> List[str]:
        """Output columns depend on computed properties, plus pass/fail flags."""
        return self._properties_to_compute + ['distribution_success', 'distribution_failures']
    ...
    def process(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Compute property + token masks, flag pass/fail, optionally drop failures.
    ...
    def _apply_distribution_filters(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Evaluate property and token filters on every molecule, flag pass/fail, and
        optionally drop the failures.

        Each filter is evaluated independently on every row (rare-token frequencies
        over the full input set), so ``distribution_failures`` lists every filter a
        molecule violates. A molecule passes (``distribution_success=True``) when it
        satisfies all active property thresholds and token filters. With ``dropna``
        False, all molecules are kept and the decision is recorded in the
        ``distribution_success`` / ``distribution_failures`` columns.
        """
        ...
        # Show initial distribution statistics on the full set
        self._show_distribution_statistics(df, "INITIAL DISTRIBUTION")

        # Property-threshold masks + per-molecule low/high violation reasons.
        property_pass, property_reasons, filter_results = self._compute_property_masks(df_original)
        if filter_results:
            self._show_filter_summary(filter_results, initial_count)

        # Token masks + per-molecule token violations.
        token_pass, token_reasons, token_results = self._compute_token_masks(df_original)
        if token_results:
            self._show_token_filter_summary(token_results, initial_count)

        # Combine into a pass flag and the full set of failure reasons per molecule.
        distribution_success = property_pass & token_pass
        df['distribution_success'] = distribution_success
        df['distribution_failures'] = self._combine_failures(
            df.index, property_reasons, token_reasons
        )

        self._token_results = token_results
        self._log_curation_summary(initial_count, property_pass, token_pass, token_results)

        if self.dropna:
            df = df[df['distribution_success']]

        return df

    # ==================== Filter Mask Computation ====================

    def _compute_property_masks(
        self, df: pd.DataFrame
    ) -> Tuple[pd.Series, Dict[Any, List[str]], List[Dict]]:
        """
        Compute the combined property-threshold pass mask, per-molecule violation
        reasons, and the per-property summary used for the FILTER RESULTS table.

        Returns:
            (property_pass, reasons, filter_results) where ``property_pass`` is a
            boolean Series (True = passes all active property thresholds),
            ``reasons`` maps each index to a list of ``"<property>:low"`` /
            ``"<property>:high"`` violation strings, and ``filter_results`` is the
            per-property pass/removed summary.
        """
        property_pass = pd.Series([True] * len(df), index=df.index)
        reasons: Dict[Any, List[str]] = {idx: [] for idx in df.index}
        filter_results = []

        for prop_name, threshold_config in self.thresholds.items():
            if prop_name not in df.columns or not self._has_active_threshold(threshold_config):
                continue

            prop_mask = self._get_property_mask(df, prop_name, threshold_config)
            property_pass &= prop_mask

            # Classify low/high violations for the failure reasons.
            stats = self._statistics_cache.get(prop_name, {})
            values = df[prop_name]
            lower_bound, upper_bound = self._get_threshold_bounds(
                prop_name, threshold_config, stats.get('mean'), stats.get('std'), values
            )
            if lower_bound is not None:
                low = (values < lower_bound).to_numpy()
                for idx in values.index[low]:
                    reasons[idx].append(f"{prop_name}:low")
            if upper_bound is not None:
                high = (values > upper_bound).to_numpy()
                for idx in values.index[high]:
                    reasons[idx].append(f"{prop_name}:high")

            passed = int(prop_mask.sum())
            removed = len(df) - passed
            filter_results.append({
                'property': prop_name,
                'passed': passed,
                'removed': removed,
                'pass_rate': 100 * passed / len(df) if len(df) > 0 else 0
            })

        return property_pass, reasons, filter_results

    def _compute_token_masks(
        self, df: pd.DataFrame
    ) -> Tuple[pd.Series, Tuple[pd.Series, pd.Series], List[Dict]]:
        """
        Compute the token pass mask, per-molecule token violations, and the
        per-filter summary. Each filter is evaluated on every row, and rare-token
        frequencies are computed over the full input set. When token curation is
        disabled or no vocabulary is available, the mask is all-True.

        Returns:
            (token_pass, (unknown_violations, rare_violations), token_results),
            with the Series indexed by ``df.index``.
        """
        n = len(df)
        token_pass = pd.Series([True] * n, index=df.index)
        unknown_viol = pd.Series([False] * n, index=df.index)
        rare_viol = pd.Series([False] * n, index=df.index)
        token_results: List[Dict] = []

        if not (self.curate_tokens and self.tokens_column in df.columns):
            return token_pass, (unknown_viol, rare_viol), token_results

        # Vocabulary comes from the upstream TokenizeData actor via context.
        ...
        if not vocabulary or len(vocabulary) == 0:
            self.log("TokenizeData vocabulary is empty. Skipping token filtering.", level='WARNING')
            return token_pass, (unknown_viol, rare_viol), token_results

        unknown_pass = pd.Series([True] * n, index=df.index)
        if self.filter_unknown_tokens:
            unknown_pass = df[self.tokens_column].apply(
                lambda toks: self._all_tokens_known(toks, vocabulary)
            )
            unknown_viol = ~unknown_pass
            removed = int(unknown_viol.sum())
            data_tokens = set()
            for toks in df[self.tokens_column]:
                if isinstance(toks, list):
                    data_tokens.update(toks)
            unknown_tokens = sorted(data_tokens - set(vocabulary.tokens()))
            token_results.append({
                'filter': 'unknown_tokens',
                'passed': n - removed,
                'removed': removed,
                'pass_rate': 100 * (n - removed) / n if n else 0,
                'criterion': 'contains token(s) absent from the vocabulary',
                'tokens': unknown_tokens,
            })

        rare_pass = pd.Series([True] * n, index=df.index)
        if self.token_frequency_threshold is not None:
            rare_tokens = self._compute_rare_tokens(df)
            if rare_tokens:
                rare_pass = df[self.tokens_column].apply(
                    lambda toks: self._has_no_rare_tokens(toks, rare_tokens)
                )
                rare_viol = ~rare_pass
            removed = int(rare_viol.sum())
            token_results.append({
                'filter': 'rare_tokens',
                'passed': n - removed,
                'removed': removed,
                'pass_rate': 100 * (n - removed) / n if n else 0,
                'criterion': f'contains token(s) in <{self.token_frequency_threshold}% of molecules',
                'tokens': sorted(rare_tokens),
            })

        token_pass = unknown_pass & rare_pass
        return token_pass, (unknown_viol, rare_viol), token_results

    @staticmethod
    def _combine_failures(
        index: pd.Index,
        property_reasons: Dict[Any, List[str]],
        token_reasons: Tuple[pd.Series, pd.Series],
    ) -> pd.Series:
        """Join per-molecule violation reasons into a ``distribution_failures`` string."""
        unknown_viol, rare_viol = token_reasons
        out = []
        for idx in index:
            parts = list(property_reasons.get(idx, []))
            if bool(unknown_viol.get(idx, False)):
                parts.append("tokens:unknown")
            if bool(rare_viol.get(idx, False)):
                parts.append("tokens:rare")
            out.append("; ".join(parts))
        return pd.Series(out, index=index)

    @staticmethod
    def _all_tokens_known(tokens, vocabulary) -> bool:
        """True iff every token in ``tokens`` is present in ``vocabulary``."""
        if not isinstance(tokens, list):
            return False
        return all(token in vocabulary for token in tokens)

    @staticmethod
    def _has_no_rare_tokens(tokens, rare_tokens) -> bool:
        """True iff ``tokens`` contains no rare token."""
        if not isinstance(tokens, list):
            return False
    ...
    def _filter_by_unknown_tokens(self, df: pd.DataFrame, vocabulary) -> pd.DataFrame:
        """
        Remove molecules whose tokens are not all present in the vocabulary.

        Under a dynamically-updated vocabulary this is a no-op (every token is
        already known); it only removes molecules when a fixed/external vocabulary
        is used and the input contains out-of-vocabulary tokens.
        """
        mask = df[self.tokens_column].apply(
            lambda toks: self._all_tokens_known(toks, vocabulary)
        )
        removed = int((~mask).sum())
        if removed > 0:
            self.log(
                f"Removed {removed} molecules containing tokens absent from the vocabulary."
            )
        return df[mask].reset_index(drop=True)

    def _create_curated_vocabulary(self, df: pd.DataFrame) -> Optional[str]:
        """
        Build and save the curated vocabulary from the molecules that passed
        curation (``distribution_success == True``), and report how it differs
        from the base (reference) vocabulary.

        The curated vocabulary always describes the passing subset, regardless of
        whether failures were physically dropped (``dropna``). Relative to the base
        vocabulary it can only gain tokens ("new") when out-of-vocabulary tokens
        were retained (``filter_unknown_tokens=False``); otherwise it is a subset.

        Args:
            df: Curated DataFrame with a tokens column (and, when present, the
                ``distribution_success`` flag column).

        Returns:
            Path to the curated vocabulary file, or None if creation failed.
        """
        try:
            passed_df = df[df['distribution_success']] if 'distribution_success' in df.columns else df

            curated_tokens = extract_tokens_from_dataframe(passed_df, self.tokens_column)
            ...
            curated_set = set(curated_vocab.tokens())
            td_actor = self.get_actor(Steps.TOKENS)
            if td_actor:
                base_set = set(td_actor.get_vocabulary().tokens())
                unused = base_set - curated_set
                new = curated_set - base_set
                msg = (
                    f"Curated vocabulary: {len(curated_set)} tokens from {len(passed_df)} "
                    f"curated molecules ({len(unused)} unused, {len(new)} new "
                    f"vs base vocabulary of {len(base_set)})"
                )
                if new:
                    msg += f": {sorted(new)}"
                # 'new' tokens can only appear when out-of-vocabulary tokens were
                # retained (filter_unknown_tokens=False with a fixed vocabulary),
                # i.e. the vocabularies have diverged; flag it.
                self.log(
                    f"{msg}. Saved to {curated_vocab_file}.",
                    level='WARNING' if new else 'INFO',
                )
            else:
                self.log(
                    f"Curated vocabulary: {len(curated_set)} tokens from {len(passed_df)} "
                    f"curated molecules. Saved to {curated_vocab_file}."
                )
    ...
    def _compute_rare_tokens(self, df: pd.DataFrame) -> set:
        """
        Identify rare tokens from ``df`` (the property-and-unknown-passed subset).

        Each molecule contributes once per UNIQUE token; tokens present in fewer
        than ``token_frequency_threshold`` percent of molecules are rare. Renders
        the token distribution plot when plotting is enabled.
        """
        all_tokens = []
        ...
        token_freq_df = pd.DataFrame({
            ...
            'percent': (token_counts.values / total_molecules) * 100 if total_molecules else []
        ...
        if rare_tokens and self.plot_distributions and self.distributions_dir:
    ...
    def _log_curation_summary(self, initial_count: int, property_pass: pd.Series,
                              token_pass: pd.Series, token_results: List[Dict]) -> None:
        """Report the curation funnel and the tokens that drove token-based removals.

        The funnel decomposes the drop into a property stage and a token stage
        (token removals counted among the property-passing molecules), so the
        stages sum to the total. Under ``dropna=False`` molecules are flagged
        rather than removed, and the wording reflects that.
        """
        removed_property = int((~property_pass).sum())
        removed_token = int((property_pass & ~token_pass).sum())
        removed_total = removed_property + removed_token
        retained = initial_count - removed_total
        pct = 100 * removed_total / initial_count if initial_count else 0
        ...
        self._curation_summary = {
            ...
            'dropped': bool(self.dropna),
        }

        verb, stage = ('retained', 'removed') if self.dropna else ('pass', 'flagged')
        self.log(
            f"Distribution curation: {initial_count} → {retained} {verb} "
            f"({stage} {removed_total}, {pct:.1f}%)  |  property {removed_property}, token {removed_token}"
        )
        for r in token_results:
            tokens = r.get('tokens') or []
            if tokens:
                self.log(
                    f"  {r['filter']}: {len(tokens)} token(s) present in {r['removed']} molecules: {tokens}"
                )

    def _show_token_filter_summary(self, token_results: List[Dict], initial_count: int) -> None:
        """Display a per-filter summary of token-based molecule curation.

        Each row is one token filter, reporting how many of the ``initial_count``
        molecules pass it and how many it removes.
        """
        rows = []
        for r in token_results:
            rows.append({
                'filter': r['filter'],
                'passed': f"{r['passed']}/{initial_count}",
                'removed': r['removed'],
                'pass_rate': f"{r['pass_rate']:.1f}%",
                'criterion': r['criterion'],
            })
        summary_df = pd.DataFrame(rows)
        self.log(f"TOKEN FILTER RESULTS\n{summary_df.to_markdown(index=False)}")
    ...
    def _save_curation_results(self) -> None:
        ...
        # Curation funnel and the token-based removals, including the identities of
        # the tokens that drove them.
        if getattr(self, '_curation_summary', None):
            results['summary'] = self._curation_summary
        if getattr(self, '_token_results', None):
            results['token_curation'] = {
                r['filter']: {
                    'removed': r['removed'],
                    'pass_rate': round(r['pass_rate'], 2),
                    'tokens': r.get('tokens', []),
                }
                for r in self._token_results
            }
