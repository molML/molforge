...
class GenerateConfsParams(BaseParams):
    ...
    def _validate_params(self) -> None:
        ...
        if self.backend == 'openeye':
            ...
            if self.strict and not self.flipper:
                warnings.warn(
                    "backend='openeye' with strict=True skips molecules that have "
                    "unassigned stereocenters. Assign stereochemistry during curation, "
                    "or enable flipper, to retain them.",
                    UserWarning,
                )
    ...
    def _post_init_hook(self) -> None:
        ...
        Resolved, machine-specific values are kept in private attributes so they
        stay out of the serialized config; the user-facing ``oeomega_path`` and
        ``mpi_np`` inputs remain as provided.
        """
        if self.backend == 'openeye':
            # OpenEye: resolve the OMEGA executable and MPI worker count.
            self._resolved_oeomega_path = self.oeomega_path or self._find_omega_executable()
