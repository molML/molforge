...
class PropertyThreshold(BaseParams):
    ...
    """Absolute lower bound (fixed minimum value); highest priority threshold type."""
    max_value: Optional[float] = None
    """Absolute upper bound (fixed maximum value); highest priority threshold type."""

    # Statistical thresholds (mean ± n*std)
    statistical_lower: Optional[float] = None
    """Lower bound as mean + n*std (e.g. -2.0 for mean - 2σ)."""
    statistical_upper: Optional[float] = None
    """Upper bound as mean + n*std (e.g. 2.0 for mean + 2σ)."""

    # Quantile thresholds (percentiles)
    quantile_lower: Optional[float] = None
    """Lower bound as a percentile (e.g. 0.025 for the 2.5th percentile)."""
    quantile_upper: Optional[float] = None
    """Upper bound as a percentile (e.g. 0.975 for the 97.5th percentile)."""
...
class CurateDistributionParams(BaseParams):
    ...
    """DataFrame column containing tokenized SMILES."""

    # Property-specific thresholds
    thresholds: Dict[str, PropertyThreshold] = field(default_factory=dict)
    """Per-property threshold configuration, keyed by property name."""

    # Global threshold rules (applied to all properties without explicit thresholds)
    global_statistical_threshold: Optional[float] = None
    """Statistical threshold applied to all properties lacking explicit thresholds (e.g. 2.0 for ±2σ)."""
    global_quantile_threshold: Optional[float] = None
    """Quantile threshold applied to all properties lacking explicit thresholds (e.g. 0.025 for 2.5%-97.5%)."""

    # Property computation control
    properties: Optional[List[str]] | str = None
    """Which properties to compute and filter on: 'all' for every supported property, an explicit list of names, or None to use only the properties that have an active threshold configured."""
    ...
    """Enable token-based curation of molecules."""
    filter_unknown_tokens: bool = True
    """Remove molecules containing tokens absent from the vocabulary (requires curate_tokens=True)."""
    token_frequency_threshold: Optional[float] = None
    """Rarity cutoff as a percentage (strictly between 0 and 100): tokens present in fewer than this percent of molecules are 'rare', and molecules containing any rare token are removed. Requires curate_tokens=True."""

    # Processing options
    compute_properties: bool = True
    """Compute molecular properties before applying distribution filters."""
    dropna: bool = True
    """If True (default), drop molecules that fail any active distribution/token filter; if False, retain all molecules and record pass/fail in distribution_success / distribution_failures."""

    # Visualization
    plot_distributions: bool = True
    """Generate distribution plots for the computed properties."""
    perform_pca: bool = False
    """Run PCA on the computed properties for visualization."""
