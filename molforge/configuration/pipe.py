...
class ConstructPipe:
    """Main pipeline class using the actor registry and factory for configuration of steps."""
    ...
    def _log_error(self, actor: Any, error: Exception) -> None:
        ...
        error_msg = f"Error in {actor._nametag}: {type(error).__name__}: {error}"
    ...
    def _save_config(self) -> None:
        """Write the run configuration (canonical format plus run metadata)."""
        config = self._params.to_config()
        config.update({
    ...
    def print(self, *values, level: str = None, **kwargs):
        ...
        if self.logger and self.verbose:
            ...
            print(self._nametag, message, **kwargs)
    ...
    def get_actor(self, identifier: str) -> Optional[Any]:
        ...
        Note: Delegates to ActorRegistry.get_actor_instance().
        """
