...
class PipelineLogger:
    ...
    Provides colored output for console and timestamped logs tagged with each source's nametag.
    """
    ...
    def __init__(self, 
        ...
        self.fmt = '%(asctime)s    | %(nametag)s | %(levelname)s | %(message)s'
    ...
    def update_logger(self, log_file: str = None):
        ...
        Returns: True if now logging to a file, False if console-only.
        """
    ...
    def log(self, level: str, nametag: str, message: str, *args, **kwargs):
        """Emit a log record tagged with the source's nametag."""
        extra = {'nametag': nametag}
        getattr(self.logger, level.lower())(message, *args, extra=extra, **kwargs)

class BaseFormatter(logging.Formatter):
    """Base formatter that pads log levels to a fixed width and handles the nametag field."""
